...
def single_cer_map(f=0.4, g=0.5, gap_cost=2, seed=None, method="ICM",
			   ...
			   max_entities=None, trajectories_path=None):
	"""
	...
	sGs, actual_bbox = generate_trajectory_graphs(trajectories_path=trajectories_path, n_subgraphs=n_input_graphs)
	print 'Sofiane: There are %s subgraphs.' % len(sGs)
	similarity_tuples = util.compute_map_similarity_tuples(sGs)
	t0 = time.time()
	...
	print 'Sofiane: printing the x thing:', x
	# Sofiane: I'm removing precision/recall/fscore as I'm not computing them previousely.
	# precision, recall, fscore = other['scores']
...
def test_single_map(f=None, do_plot=False, do_save=False, title="", trajectories_path=None):
	if f is None:
		if len(sys.argv) > 1:
			f = float(sys.argv[1])
		else:
